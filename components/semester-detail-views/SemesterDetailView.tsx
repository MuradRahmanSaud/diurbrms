...
import { EnrollmentEntry, FullRoutineData, RoomEntry, DefaultTimeSlot, ProgramEntry, CourseType, SemesterSystem, RoomTypeEntry, ProgramType, DayOfWeek, ScheduleOverrides, SemesterCloneInfo, ClassDetail, BuildingEntry, FloorEntry, User } from '../../types';
...
import TeacherListView, { TeacherData } from './TeacherListView';
import CourseListView from './CourseListView';
import FullSectionListView from './FullSectionListView';
    ...
    <div className="bg-slate-100 p-2 sm:p-3 rounded-lg h-full flex flex-col relative">
      ...
      <div className="flex-grow flex flex-col min-h-0 bg-white p-3 rounded-lg shadow-md">
        ...
        <main className="flex-grow flex flex-col min-h-0 min-w-0">
            <div className="h-full flex flex-col">
                <div className="flex-shrink-0 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2 mb-3">
                    <InfoCard 
                        title="Teacher"
                        mainValue={stats.teacherStats.total}
                        ...
                        gradientClasses={gradients[7]}
                        onClick={() => { setInternalView('teacherList'); setExpandedTeacherId(null); }}
                    />
                    <InfoCard 
                        title="Unique Courses"
                        mainValue={stats.courseStats.total}
                        icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>}
                        gradientClasses={gradients[0]}
                        onClick={() => setInternalView('courseList')}
                    />
                    <InfoCard 
                        title="Total Section"
                        mainValue={stats.sectionStats.total}
                        icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" /></svg>}
                        gradientClasses={gradients[1]}
                        onClick={() => setInternalView('sectionList')}
                    />
                    <InfoCard 
                        title="Slot Requirement"
                        mainValue={stats.weeklyClassStats.total}
                    ...
                    <InfoCard 
                        title="Total Room"
                        mainValue={stats.roomStats.total}
                        ...
                        onClick={() => setInternalView('roomList')}
                    />
                    <InfoCard 
                        title="Total Slot"
                        mainValue={(stats.totalSlotStats.total).toLocaleString()}
                    ...
                    <InfoCard 
                        ...
                        mainValue={stats.bookedSlotStats.total.toLocaleString()}
                    ...
                    <InfoCard 
                        title="Slot Usage"
                        mainValue={`${stats.bookedSlotStats.total.toLocaleString()} / ${stats.totalSlotStats.total.toLocaleString()}`}
                        icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>}
                        gradientClasses={gradients[6]}
                    />
                </div>
                <div className="flex-grow bg-white rounded-lg shadow-inner border border-gray-200 p-2 flex flex-col min-h-0">
                    {internalView === 'overview' ? (
                        <div className="flex-grow min-h-0 overflow-auto custom-scrollbar -m-1 p-1">
                            <table className="min-w-full table-fixed border-separate" style={{ borderSpacing: '3px' }}>
                                <thead className="sticky top-0 bg-white z-10">
                                    <tr>
                                        ...
                                        {routineOverviewData.headerSlotsForGrid.map(slot => {
                                            ...
                                            return (
                                                <th key={slot.id} className="w-24 p-1 text-center text-[9px] font-bold text-white uppercase bg-teal-700 rounded-md shadow-sm break-words">
                                        ...
                                        <th className="w-24 p-1 text-center text-[11px] font-bold text-white uppercase bg-slate-700 rounded-md shadow-sm">Daily Summary</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {routineOverviewData.activeDaysForGrid.map(day => {
                                        const daySummary = routineOverviewData.daySummaries[day];
                                        return (
                                            <tr key={day}>
                                                <th className="p-1.5 text-xs font-bold text-white bg-teal-600 rounded-md shadow-sm h-12">{day}</th>
                                                {routineOverviewData.headerSlotsForGrid.map(slot => {
                                                    ...
                                                    let bgColor = 'bg-gray-100'; let textColor = 'text-gray-500'; let progressBgColor = 'bg-gray-300'; let borderColor = 'border-gray-200'; let hoverBgColor = 'hover:bg-gray-200';
                                                    if (total > 0) {
                                                    ...
                                                    return (
                                                        <td key={slot.id} className="p-0.5 h-12" onClick={() => total > 0 && handleOpenSlotDetail(day, slot)}>
                                                            <div className={`h-full w-full rounded-md shadow-sm transition-all duration-150 p-2 flex flex-col justify-between border ${borderColor} ${bgColor} ${total > 0 ? `cursor-pointer ${hoverBgColor}` : 'cursor-not-allowed'}`}>
                                                                <div className="flex justify-between items-baseline">
                                                                    <span className={`font-bold text-lg ${textColor}`}>{booked}</span>
                                                                    <span className={`text-xs font-medium ${textColor}`}>/{total}</span>
                                                                </div>
                                                                <div>
                                                                    <div className="text-right text-[10px] font-semibold text-gray-500 mb-1">{total > 0 ? `${(occupancy * 100).toFixed(0)}% Full` : 'N/A'}</div>
                                                                    <div className="w-full bg-gray-200 rounded-full h-1"><div className={`${progressBgColor} h-1 rounded-full`} style={{ width: `${occupancy * 100}%` }}></div></div>
                                                                </div>
                                                            </div>
                                                        </td>
                                                    );
                                                })}
                                                <td className="p-0.5 h-12">
                                ...
                                <tfoot className="sticky bottom-0 bg-white/80 backdrop-blur-sm">
                                  <tr>
                                    <th className="p-1.5 text-xs font-bold text-white bg-slate-800 rounded-md shadow-sm h-12">Total</th>
                                    {routineOverviewData.columnTotals.map((colTotal, index) => (
                                      <td key={`total-${index}`} className="p-0.5 h-12">
                                        <SummaryCell booked={colTotal.booked} total={colTotal.total} />
                                      </td>
                                    ))}
                                    <td className="p-0.5 h-12">
                                      <SummaryCell booked={routineOverviewData.grandTotal.booked} total={routineOverviewData.grandTotal.total} isGrandTotal={true} />
                                    </td>
                                  </tr>
                                </tfoot>
                              </table>
                        </div>
                    ) : internalView === 'teacherList' ? (
                        <TeacherListView 
                            teachers={paginatedTeachers}
                            allPrograms={allPrograms}
                            onBack={() => { setInternalView('overview'); setExpandedTeacherId(null); }}
                            expandedTeacherId={expandedTeacherId}
                            onToggleExpand={(teacherId) => setExpandedTeacherId(prev => prev === teacherId ? null : teacherId)}
                            ciwCounts={ciwCounts}
                            ...
                            onNextPage={() => setTeacherListPage(p => Math.min(p + 1, totalTeacherPages))}
                            onPrevPage={() => setTeacherListPage(p => Math.max(1, p - 1))}
                            totalTeacherCount={filteredTeacherList.length}
                            searchTerm={teacherSearchTerm}
                            onSearchChange={setTeacherSearchTerm}
                            uniqueDesignations={uniqueDesignations}
                            designationFilter={designationFilter}
                            onDesignationFilterChange={handleDesignationFilterChange}
                            creditLoadFilter={creditLoadFilter}
                            onCreditLoadFilterChange={handleCreditLoadFilterChange}
                            onResetFilters={handleResetTeacherFilters}
                            activeFilterCount={activeTeacherFilterCount}
                            programFilter={programFilter}
                            onProgramFilterChange={handleTeacherProgramFilterChange}
                            uniqueProgramsForFilter={uniqueProgramsForTeacherFilter}
                            onViewTeacherDetail={handleViewTeacherDetail}
                        />
                    ) : internalView === 'courseList' ? (
                         <CourseListView
                            uniqueCourses={paginatedCourses}
                            totalCourseCount={filteredCourseList.length}
                            onBack={() => setInternalView('overview')}
                            ciwCounts={ciwCounts}
                            classRequirementCounts={classRequirementCounts}
                            currentPage={courseListPage}
                            totalPages={totalCoursePages}
                            onNextPage={() => setCourseListPage(p => Math.min(p + 1, totalCoursePages))}
                            onPrevPage={() => setCourseListPage(p => Math.max(1, p - 1))}
                            searchTerm={courseSearchTerm}
                            onSearchChange={setCourseSearchTerm}
                            onResetFilters={handleResetCourseFilters}
                            uniqueLevelTerms={courseUniqueLevelTerms}
                            levelTermFilter={courseLevelTermFilter}
                            onLevelTermFilterChange={handleCourseLevelTermFilterChange}
                            uniqueCourseTypes={courseUniqueCourseTypes}
                            courseTypeFilter={courseTypeFilter}
                            onCourseTypeFilterChange={handleCourseTypeFilterChange}
                            uniqueCredits={courseUniqueCredits}
                            creditFilter={courseCreditFilter}
                            onCreditFilterChange={handleCourseCreditFilterChange}
                            minMaxFilters={courseMinMaxFilters}
                            onMinMaxFilterChange={handleCourseMinMaxFilterChange}
                            setCoursesData={setCoursesData}
                            onUpdateLevelTerm={onUpdateLevelTerm}
                            onUpdateWeeklyClass={onUpdateWeeklyClass}
                            onUpdateCourseType={onUpdateCourseType}
                        />
                    ) : internalView === 'roomList' ? (
                        <RoomListView 
                            rooms={filteredRooms.sort((a,b) => a.roomNumber.localeCompare(b.roomNumber))}
                            onBack={() => setInternalView('overview')}
                        ...
                        <FullSectionListView
                            user={user}
                            coursesData={sectionsForSemester}
                            onClose={() => setInternalView('overview')}
                            allPrograms={allPrograms}
                            ...
                            classRequirementCounts={classRequirementCounts}
                            setCoursesData={setCoursesData}
                            dashboardTabFilter={activeTab}
                            allRooms={allRooms}
                            systemDefaultSlots={systemDefaultSlots}
       ...
       {isSlotDetailModalOpen && selectedSlotData && (
          <>
            <div className="absolute inset-0 bg-black bg-opacity-60 z-40 transition-opacity duration-300 ease-out" onClick={handleCloseSlotDetail} aria-hidden="true"></div>
            <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
              <DayTimeSlotDetailModal isOpen={isSlotDetailModalOpen} onClose={handleCloseSlotDetail} day={selectedSlotData.day} selectedSlotObject={selectedSlotData.slot} systemDefaultSlots={systemDefaultSlots} fullRoutineData={routineData} roomEntries={filteredRooms} onRoomNameClick={(room) => {}} getProgramShortName={getProgramShortName} activeProgramIdInSidebar={selectedProgramId} scheduleOverrides={scheduleOverrides} selectedDate={null} allSemesterConfigurations={allSemesterConfigurations} allPrograms={allPrograms} />
            </div>
          </>
        )}
        <TeacherViewDetailModal
            isOpen={!!teacherForDetailModal}
            onClose={() => setTeacherForDetailModal(null)}
            teacher={teacherForDetailModal}
        ...
        />
         {selectedRoomForModal && (
            <RoomDetailModal
                ...
                getFloorName={getFloorName}
                getCategoryName={getCategoryNameFromContext}
                getTypeName={getTypeName}
                getProgramShortName={getProgramShortName}
                fullRoutineData={fullRoutineData} 
                systemDefaultSlots={systemDefaultSlots} 
